...
class View(QGraphicsView):
    ...
    def _scene_ctx_for(self, view_event, item):
        ...
        gp = view_event.globalPos()
        class _Ctx:
            _sp = self.mapToScene(view_event.pos())
    ...
    def open_path(self, path):
        """Open a .flow file by path — called from QFileOpenEvent or sys.argv."""
        if not self._ask_save_if_dirty(): return
        # Force logo fade-out immediately regardless of animation state
        if hasattr(self, "_logo"):
            self._logo._opacity = 1.0  # treat as fully visible so fade-out triggers
